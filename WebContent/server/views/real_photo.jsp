            ...
            <div class="layui-card">
            	<div class="table-responsive">
			 		   <table class="layui-table" lay-skin="line" lay-size="lg" id="table">						  
						  ...
						  <tbody>
						  <c:forEach items="${real.list }" var="real">
						  	  <tr>
							      <td>${real.rid}</td>
							      <td>						      	  
							      	 <div id="layer-photos-demo" class="layer-photos-demo">
									      <img layer-pid="" layer-src="${ctx}/static/${real.realimg}" src="${ctx}/static/${real.realimg}" alt="图片名">
									  </div>
							      </td>
							      <td class="layui-form">
							         ${real.realname }
							      ...
							      <td class="layui-form">
							      <fmt:formatDate value="${real.time }" pattern="yyyy-MM-dd"/>
							      </td>					     
							      <td>
							      	  <div class="layui-table-cell laytable-cell-1-0-10"> 							      	  	 
								      	  	<a class="layui-btn layui-btn-normal layui-btn-xs"
								      	  	onclick="but_update('${real.rid}')">修改</a> 
								      	  	<a class="layui-btn layui-btn-danger layui-btn-xs delete"
								      	  	onclick="but_delete('${real.rid}')">删除</a> 
							      	  </div>
							      </td>
							    </tr>
						  </c:forEach>
           		...
           		<div id="div_pagination_bottom">
			            <nav aria-label="Page navigation">
			                <ul class="pagination">
			                     <li>
			                         <a href="${ctx}/getRealAll?currpage=${real.pageNum-1}"
			                           aria-label="Previous">
			                             <span aria-hidden="true">&laquo;</span>
			                         </a>
			                     </li>
			                     <c:forEach items="${real.navigatepageNums }" var="page_Num">
			                         <c:if test="${page_Num == real.pageNum }">
			                             <li class="active"><a href="#" style="margin-top: 7px;">${ page_Num}</a></li>
			                        </c:if>
			                         <c:if test="${page_Num != real.pageNum }">
			                            <li>
			                                 <a style="margin-top: 7px;" href="${ctx}/getRealAll?currpage=${ page_Num}">${ page_Num}</a>
			                            </li>
			                        </c:if>
			                     </c:forEach>
			                    <li>
    ...
    <script>
		...
		$('#addbanner').on('click', function(){			
          ...
          layer.open({
                ...
                shade: 0.8,
                area: ['660px', '420px'],
                content: 'server/views/real_add.jsp',
                end: function () {
                   location.reload();
                }
            });
      ...
      function but_update(id){
		...
		function but_delete(id){
				 $.get("${ctx}/deleterealById",{rid:id},function(res){
					 if (res.code>0) {
