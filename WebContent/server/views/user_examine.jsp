            	...
            	<div class="table-responsive">
			 		  ...
			 		  <table class="layui-table" lay-skin="line" lay-size="lg" id="table">						  
						 ...
						 <c:forEach items="${pageInfo.list }" var="users" varStatus="st">
						    	<tr>
							      ...
							      <td>${users.address }</td>
							       <td style="color: red;">
									<c:if test="${users.isok==1 }">
														未通过
								   </c:if>
									<c:if test="${users.isok==2 }">
														通过
									</c:if>
							      ...
							      <button class="layui-btn layui-btn-normal layui-btn-xs "  onclick="update(${users.uid })">
								      	  <i class="icon-trash bigger-120"></i>审核</button>
							      </td>
							      <td class="delete" >
           		...
           		<div id="div_pagination_bottom">
			            <nav aria-label="Page navigation">
			                <ul class="pagination">
			                     <li>
			                         <a href="${pageContext.request.contextPath}/examine?currpage=${pageInfo.pageNum-1}"
			                           aria-label="Previous">
			                     ...
			                     <c:forEach items="${pageInfo.navigatepageNums }" var="page_Num">
			                         ...
			                         <c:if test="${page_Num != pageInfo.pageNum }">
			                            <li>
			                                 <a style="margin-top: 7px;" href="${pageContext.request.contextPath}/examine?currpage=${ page_Num}">${ page_Num}</a>
			                            </li>
			                        </c:if>
			                     </c:forEach>
			                    <li>
  			...
  			layui.use('layer', function(){
  				alert("确定修改信息？");
  	              ...
  	              layer.open({
  	                    ...
  	                    shade: 0.8,
  	                    area: ['660px', '420px'],
  	                    content: "server/views/user_isOk.jsp",
  	                    end: function () {
  	                       location.reload();
  	                    }
  	                });
