...
<body>
<div class="body-wrapper" style="margin-top: -10px;">
	<%@ include file="top1.jsp"%>
<div class="light-wrapper" >
    <div class="container inner">
      <h2 class="section-title text-center">婚礼</h2>
	</div>
  </div>
  <section class="destination-section pt-120 pb-120" style="margin-top: -100px;">
    <div class="container">
      <div class="row mb-none-30">
        <c:forEach items="${promarry.list }" var="marry">
        <div class="col-lg-4 col-sm-6">
          <div class="destination-item mb-30">
            <div class="thumb">
            <a href="${ctx}/getmarryByid?mid=${marry.mid}">
              <img src="${ctx}/static/${marry.idpicpath}" alt="image"></a>
            </div>
            <div class="content">
              <h3 class="place-name"> <a href="${ctx}/getmarryByid?mid=${marry.mid}">
              ${marry.mname }</a></h3>
              <div class="info">
                <span ><i class="budicon-cash-yen"></i>价格: ${marry.mprice}</span>
                <span>
		            ...
		            <c:if test="${loginUser.uid != null }">
		            	<a href="${ctx }/addcart?op=mid&id=${marry.mid}&uid=${loginUser.uid }" class="more">加入购物车</a> 
					 	<a href="${ctx }/addcart?op=mid&id=${marry.mid}&uid=${loginUser.uid }"><i class="budicon-shopping-cart"></i></a> 
		            </c:if></span>
		            <span>${marry.mdesc}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="pagination">
        <ul>
          <li><a href="${ctx}/getmarryList?currpage=${promarry.pageNum-1}">Prev</a></li>
          <c:forEach items="${promarry.navigatepageNums }" var="page_Num">
			 <c:if test="${page_Num == promarry.pageNum }">
			    <li class="active"><a href="#">${page_Num}</a></li>
			 </c:if>
			 <c:if test="${page_Num != promarry.pageNum }">
			 <li>
			   <a href="${ctx}/getmarryList?currpage=${page_Num}">${page_Num}</a>
			 </li>
			</c:if>
		</c:forEach>
          <li><a href="${ctx}/getmarryList?currpage=${promarry.pageNum+1}">Next</a></li>
        </ul>
      </div>
    </div>
  </section>
